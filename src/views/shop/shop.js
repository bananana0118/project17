...
async function render() {
    const categoryList = await Api.get(
        `/api/shop/product/get/category/?=${productCa}`
    );
...
// 상품 이름, 가격, 할인 가격 불러오기
async function handleProductList() {
    const productList = await Api.get("/api/shop/product/productlist");
    console.log(productList);
    productList.forEach(function (product) {
        const {
            _id,
        ...
        itemBox.innerHTML += `<li class="item" id="item-1">
                                ...
                                <div class="description">
                                    <strong class="name">${productName}</strong>
                                    <ul class="product-description">
                                        <li class="price">원가 : KRW ${addCommas(
                                            productPrice
                                        )}</li>
                                        <li class="sale">할인가 : KRW ${addCommas(
                                            productSalePrice
                                        )}</li>
                                    </ul>
                                </div>
